...
from paddle.fluid.initializer import XavierInitializer as xavier_uniform_
from paddle.fluid.initializer import ConstantInitializer as constant_
...
TSN Configurations:
    ...
    def _prepare_tsn(self, num_class):
        feature_dim =1536
        if self.dropout == 0:
            setattr(self.base_model, self.base_model.last_layer_name, fluid.dygraph.Linear(feature_dim, num_class))
            self.new_fc = None
        else:
            setattr(self.base_model, self.base_model.last_layer_name, fluid.dygraph.Dropout(p=self.dropout))
            self.new_fc =fluid.dygraph.Linear(feature_dim, num_class)
    ...
    def _prepare_base_model(self, base_model):
        ...
        if 'resnet' in base_model or 'vgg' in base_model:
            import tf_model_zoo
            self.base_model = getattr(tf_model_zoo, base_model)(True)
    ...
    def get_optim_policies(self):
        ...
        for m in self.base_model.state_dict():
            ...
            if isinstance(m, paddle.fluid.dygraph.Conv2D):
                
                ps = list(m.state_dict())
                conv_2d_cnt += 1
                normal_weight.append(ps[0])
                if len(ps) == 2:
                    normal_bias.append(ps[1])
               
            elif isinstance(m,paddle.fluid.dygraph.Conv3D):
                ps = list(m.state_dict())
                conv_3d_cnt += 1
                if conv_3d_cnt == 1:
            ...
            elif isinstance(m, paddle.fluid.dygraph.Linear):
                ps = list(m.state_dict())
                normal_weight.append(ps[0])
                if len(ps) == 2:
                    normal_bias.append(ps[1])
            # (BatchNorm1d or BatchNorm2d) params will be append to list: bn, total num 2 (enabled pbn, so only: 1st BN layer's weight + 1st BN layer's bias)
            elif isinstance(m, paddle.fluid.dygraph.BatchNorm):
                ...
                if not self._enable_pbn or bn_cnt == 1:
                    bn.extend(list(m.parameters()))
            elif len(self.base_model.state_dict()) == 0:
    ...
    def forward(self, inputs):
        ...
        if self.reshape:
            ...
            if self.base_model_name == 'C3DRes18':
                ...
                base_out = paddle.fluid.layers.reshape(base_out,(-1, self.num_segments) + base_out.shape[1:])
    ...
    def _construct_diff_model(self, base_model, keep_rgb=False):
        ...
        if not keep_rgb:
            ...
            new_kernel_size = kernel_size[:1] + (3 * self.new_length,) + kernel_size[2:]
            new_kernels = tensor.concat((params[0].data, params[0].data.mean(dim=1, keepdim=True).expand(new_kernel_size).contiguous()),
                                    1)
    ...
    def get_augmentation(self):
        if self.modality == 'RGB':
            return Compose([GroupMultiScaleCrop(self.input_size, [1, .875, .75, .66]),
                                                   GroupRandomHorizontalFlip(is_flow=False)])
        elif self.modality == 'Flow':
            return Compose([GroupMultiScaleCrop(self.input_size, [1, .875, .75]),
...
if __name__ == '__main__':
    with fluid.dygraph.guard():
        network =  TSN(101, 24, 'scratch', 'RGB',
                base_model='ECOfull',
                consensus_type='identity', dropout=0)
        img = np.zeros([384, 3, 224, 224]).astype('float32')
